...
- **"`1950 Commonwealth` returns all 19 blocks, but `122 Commonwealth` narrows to a tight 2."** The instinct was "long arterials can't narrow." Wrong — the two addresses diverge at the **neighborhood rung**, not the structure. *Both* fail cross-street narrowing identically: the geocoder snaps a mid-block address to its nearest *centerline*, which is the alley/driveway behind the building (`122` → "Public Alley No. 435", `1950` → "Chestnut Hill Drwy"), never a Commonwealth Ave corner. So both fall to neighborhood narrowing — which was an **exact string compare**. SAM returns `"Back Bay"` (== CSV `dist_name`, so 122 narrowed) but `"Brighton"` (≠ CSV `"Allston/Brighton"`, so 1950 emptied that rung and dropped to whole-street). Fix: containment match (`d.includes(n) || n.includes(d)`), excluding the `"Multiple"` catch-all district. The tell was that 122's tightness was *luck* — Back Bay happens to contain exactly one Commonwealth Ave segment; the narrowing was never as precise as it looked. **Lesson: when two similar inputs behave differently, diff their actual signal values before theorizing about structure.**
...
4. **Address narrowing doesn't pin on coarse/large streets** — `100 Boylston` (5 rows), `200 Dorchester Ave` (7), `100 Mass Ave` (12), `1950 Commonwealth` (11 after the neighborhood-containment fix; was 19). On these the nearest-intersection signal is an alley/driveway centerline so cross-street narrowing empties, and the address falls back to neighborhood narrowing — which is only as tight as the neighborhood is small on that street (Back Bay = 1 Commonwealth segment → tight; Allston/Brighton = ~8 → coarse). Neighborhood will never pin a single block on a long street; it's a backstop, not the answer. This is the **Phase 2 ArcGIS exact-segment** candidate (layer 3 address ranges → segment → layer 8 endpoints), feature-flagged for A/B comparison. See `sam-api-reference.md` for the ArcGIS layers.
